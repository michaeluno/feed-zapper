...
abstract class FeedZapper_AdminPageFramework_Form_View___Fieldset_Base extends FeedZapper_AdminPageFramework_Form_Utility {
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + FeedZapper_AdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new FeedZapper_AdminPageFramework_Form_View___Script_Widget;
        ...
        new FeedZapper_AdminPageFramework_Form_View___Script_Utility;
        new FeedZapper_AdminPageFramework_Form_View___Script_OptionStorage;
        new FeedZapper_AdminPageFramework_Form_View___Script_AttributeUpdator;
        new FeedZapper_AdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new FeedZapper_AdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .feed-zapper-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .feed-zapper-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .feed-zapper-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .feed-zapper-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .feed-zapper-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updateFeedZapper_AdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function ___getRepeatableButtonHTML($sFieldsContainerID, array $aArguments, $iFieldCount, $bSmall = true) {
        $_oFormatter = new FeedZapper_AdminPageFramework_Form_Model___Format_RepeatableField($aArguments, $this->oMsg);
    ...
    private function ___getContainerAttributes($aArguments) {
        $_aContainerAttributes = array('class' => $this->getClassAttribute('feed-zapper-repeatable-field-buttons', !empty($aArguments['disabled']) ? 'disabled' : ''),);
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enableFeedZapper_AdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='feed-zapper-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
class FeedZapper_AdminPageFramework_Form_View___Fieldset extends FeedZapper_AdminPageFramework_Form_View___Fieldset_Base {
    public function get() {
        $_aOutputs = array();
        $_oFieldError = new FeedZapper_AdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aFieldset['_section_path_array'], $this->aFieldset['_field_path_array'], $this->aFieldset['error_message']);
        $_aOutputs[] = $_oFieldError->get();
        $_oFieldsFormatter = new FeedZapper_AdminPageFramework_Form_Model___Format_Fields($this->aFieldset, $this->aOptions);
    ...
    private function _getEachFieldOutput($aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
        ...
        $_oSubFieldFormatter = new FeedZapper_AdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
    ...
    private function _getFieldOutput($sContent, $aField, $bIsLastElement) {
        $_oFieldAttribute = new FeedZapper_AdminPageFramework_Form_View___Attribute_Field($aField);
    ...
    private function _getFinalOutput($aFieldset, array $aFieldsOutput, $iFieldsCount) {
        $_oFieldsetAttributes = new FeedZapper_AdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
    ...
    private function _getEmbeddedFieldTitle($aFieldset) {
        ...
        $_oFieldTitle = new FeedZapper_AdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
    ...
    private function _getChildFieldTitle($aFieldset) {
        ...
        $_oFieldTitle = new FeedZapper_AdminPageFramework_Form_View___FieldTitle($aFieldset, array('feed-zapper-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        return $_oFieldTitle->get();
    }
    private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
        ...
        $_oFieldsAttributes = new FeedZapper_AdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
    ...
    private function _getExtras($aField, $iFieldsCount) {
        $_aOutput = array();
        $_oFieldDescription = new FeedZapper_AdminPageFramework_Form_View___Description($aField['description'], 'feed-zapper-fields-description');
