...
class FeedZapper_AdminPageFramework_Form_View___Script_CollapsibleSection extends FeedZapper_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeFeedZapper_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'feed-zapper_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeFeedZapper_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeFeedZapper_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.feed-zapper-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.feed-zapper-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.feed-zapper-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.feed-zapper-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.feed-zapper-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.feed-zapper-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.feed-zapper-collapsible-sections-title, .feed-zapper-collapsible-section-title' )
            .enableFeedZapper_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.feed-zapper-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'feed-zapper-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.feed-zapper-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.feed-zapper-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.feed-zapper-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.feed-zapper-sectionset' ).siblings().andSelf().find( '> .feed-zapper-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.feed-zapper-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .feed-zapper-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.feed-zapper-sections' ).children( '.feed-zapper-section' ).children( '.feed-zapper-section-table' ).children( 'caption' ).children( '.feed-zapper-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableFeedZapper_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'feed-zapper-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'feed-zapper-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.feed-zapper-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .feed-zapper-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.feed-zapper-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='feed-zapper-collapsible-toggle-all-button-container'>" . "<span class='feed-zapper-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
