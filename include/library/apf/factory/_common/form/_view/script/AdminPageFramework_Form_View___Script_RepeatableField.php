...
class FeedZapper_AdminPageFramework_Form_View___Script_RepeatableField extends FeedZapper_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateFeedZapper_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ];
...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.feed-zapper-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.feed-zapper-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.feed-zapper-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.feed-zapper-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.         
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            ...
            $( this ).addFeedZapper_AdminPageFrameworkRepeatableField();
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removeFeedZapper_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.feed-zapper-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.feed-zapper-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addFeedZapper_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addFeedZapper_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.feed-zapper-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.feed-zapper-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.feed-zapper-repeatable-field-buttons' );
            $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.feed-zapper-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.feed-zapper-repeatable-field-buttons' ).last();
         ...
         */
        nodeNewField.updateFeedZapper_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.feed-zapper-field' ).addBack().trigger( 
            'feed-zapper_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.feed-zapper-fields' )    // model container
            ]
        );
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.feed-zapper-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.feed-zapper-field,.feed-zapper-fields,.feed-zapper-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeFeedZapper_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.feed-zapper-field' );
        var nodeFieldsContainer = $( this ).closest( '.feed-zapper-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.feed-zapper-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.feed-zapper-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.aFeedZapper_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
