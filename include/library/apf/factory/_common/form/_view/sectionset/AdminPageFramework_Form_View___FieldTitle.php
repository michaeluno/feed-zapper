...
class FeedZapper_AdminPageFramework_Form_View___FieldTitle extends FeedZapper_AdminPageFramework_Form_Utility {
    ...
    public function get() {
        ...
        $_oInputTagIDGenerator = new FeedZapper_AdminPageFramework_Form_View___Generate_FieldInputID($aField, 0);
        $_aLabelAttributes = array('class' => $this->getClassAttribute('feed-zapper-field-title', $this->aClassSelectors), 'for' => $_oInputTagIDGenerator->get(),);
    ...
    private function _getFieldOutputsInFieldTitleAreaFromNestedFields($aField) {
        ...
        foreach ($aField['content'] as $_aNestedField) {
            ...
            $_oFieldset = new FeedZapper_AdminPageFramework_Form_View___Fieldset($_aNestedField, $this->aSavedData, $this->aFieldErrors, $this->aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
            $_sOutput.= $_oFieldset->get();
        }
        return $_sOutput;
    }
    private function _getToolTip($asTip, $sElementID) {
        $_oToolTip = new FeedZapper_AdminPageFramework_Form_View___ToolTip($asTip, $sElementID);
    ...
    private function _getDebugInfo($aField) {
        ...
        $_oToolTip = new FeedZapper_AdminPageFramework_Form_View___ToolTip(array('title' => $this->oMsg->get('field_arguments'), 'dash-icon' => 'dashicons-info', 'icon_alt_text' => '[' . $this->oMsg->get('debug') . ' ]', 'content' => FeedZapper_AdminPageFramework_Debug::getDetails($aField) . '<span class="feed-zapper-info">' . $this->getFrameworkNameVersion() . '  (' . $this->oMsg->get('debug_info_will_be_disabled') . ')' . '</span>', 'attributes' => array('container' => array('class' => 'debug-info-field-arguments'),)), $aField['field_id'] . '_debug');
        return $_oToolTip->get();
    }
