...
class FeedZapper_AdminPageFramework_Form_View___Sectionsets extends FeedZapper_AdminPageFramework_Form_View___Section_Base {
    ...
    public function get() {
        $_oFormatSectionsetsByTab = new FeedZapper_AdminPageFramework_Form_View___Format_SectionsetsByTab($this->aStructure['sectionsets'], $this->aStructure['fieldsets'], $this->aArguments['nested_depth']);
        $_aOutput = array();
        ...
        $_oDebugInfo = new FeedZapper_AdminPageFramework_Form_View___DebugInfo($this->aArguments['structure_type'], $this->aCallbacks, $this->oMsg);
        $_sOutput = implode(PHP_EOL, $_aOutput);
        $_sElementID = "feed-zapper-sectionsets-" . uniqid();
        return $this->_getSpinnerOutput($_sOutput) . "<div id='{$_sElementID}' class='feed-zapper-sctionsets feed-zapper-form-js-on'>" . $_sOutput . FeedZapper_AdminPageFramework_Form_View___Script_SectionTab::getEnabler() . $_oDebugInfo->get() . "</div>";
    }
    private function _getSpinnerOutput($_sOutput) {
        if (trim($_sOutput)) {
            return "<div class='feed-zapper-form-loading' style='display: none;'>" . $this->oMsg->get('loading') . "</div>";
        }
        return '';
    }
    private function _getFormOutput(array $aSectionsets, array $aFieldsets, $sSectionTabSlug, $aCallbacks) {
        ...
        return $_sSectionSet ? "<div " . $this->getAttributes(array('class' => 'feed-zapper-sectionset', 'id' => "sectionset-{$sSectionTabSlug}_" . md5(serialize($aSectionsets)),)) . ">" . $_sSectionSet . "</div>" : '';
    }
    ...
    private function _getCollapsibleArgumentForSections(array $aSectionset = array()) {
        $_oArgumentFormater = new FeedZapper_AdminPageFramework_Form_Model___Format_CollapsibleSection($aSectionset['collapsible'], $aSectionset['title'], $aSectionset);
    ...
    private function _getSectionsetTable($_aOutputs, $_sSectionsID, array $_aSection, array $aFieldsInSections) {
        ...
        $_oEachSectionArguments = new FeedZapper_AdminPageFramework_Form_Model___Format_EachSection($_aSection, null, array(), $_sSectionsID);
    ...
    private function _getSubSections($_aOutputs, $_sSectionsID, $_aSection, $_aSubSections) {
        if (!empty($_aSection['repeatable'])) {
            $_aOutputs['section_contents'][] = FeedZapper_AdminPageFramework_Form_View___Script_RepeatableSection::getEnabler($_sSectionsID, $_aOutputs['count_subsections'], $_aSection['repeatable'], $this->oMsg);
            $_aOutputs['section_contents'][] = $this->_getRepeatableSectionFlagTag($_aSection);
        }
        if (!empty($_aSection['sortable'])) {
            $_aOutputs['section_contents'][] = FeedZapper_AdminPageFramework_Form_View___Script_SortableSection::getEnabler($_sSectionsID, $_aSection['sortable'], $this->oMsg);
        ...
        foreach ($_aSubSections as $_iIndex => $_aFields) {
            $_oEachSectionArguments = new FeedZapper_AdminPageFramework_Form_Model___Format_EachSection($_aSection, $_iIndex, $_aSubSections, $_sSectionsID);
    ...
    private function _getSectionTableWithTabList(array $_aOutputs, array $aSectionset, $aFieldsetsPerSection) {
        ...
        $_oSectionTable = new FeedZapper_AdminPageFramework_Form_View___Section($this->aArguments, $aSectionset, $this->aStructure, $aFieldsetsPerSection, $this->aSavedData, $this->aFieldErrors, $this->aStructure['field_type_definitions'], $this->aCallbacks, $this->oMsg);
    ...
    private function _getFormattedSectionsTablesOutput(array $aOutputs, $aSectionset, $sSectionsID, array $aCollapsible, $sSectionTabSlug) {
        ...
        $_oCollapsibleSectionTitle = new FeedZapper_AdminPageFramework_Form_View___CollapsibleSectionTitle(array('title' => $this->getElement($aCollapsible, 'title', ''), 'tag' => 'h3', 'section_index' => null, 'collapsible' => $aCollapsible, 'container_type' => 'sections', 'sectionset' => $aSectionset,), array(), $this->aSavedData, $this->aFieldErrors, $this->aStructure['field_type_definitions'], $this->oMsg, $this->aCallbacks);
        $_oSectionsTablesContainerAttributes = new FeedZapper_AdminPageFramework_Form_View___Attribute_SectionsTablesContainer($aSectionset, $sSectionsID, $sSectionTabSlug, $aCollapsible, $aOutputs['count_subsections']);
    ...
    private function _getSectionTabList($sSectionTabSlug, array $aSectionTabList) {
        return $sSectionTabSlug ? "<ul class='feed-zapper-section-tabs nav-tab-wrapper'>" . implode(PHP_EOL, $aSectionTabList) . "</ul>" : '';
    }
    private function _getTabList(array $aSection, array $aFields, $hfFieldCallback) {
        ...
        $_aTabAttributes = $aSection['attributes']['tab'] + array('class' => 'feed-zapper-section-tab nav-tab', 'id' => "section_tab-{$_sSectionTagID}", 'style' => null);
        ...
        $_oSectionTitle = new FeedZapper_AdminPageFramework_Form_View___SectionTitle(array('title' => $aSection['title'], 'tag' => 'h4', 'section_index' => $iSectionIndex, 'sectionset' => $aSection,), $aFields, $this->aSavedData, $this->aFieldErrors, $this->aStructure['field_type_definitions'], $this->oMsg, $this->aCallbacks);
