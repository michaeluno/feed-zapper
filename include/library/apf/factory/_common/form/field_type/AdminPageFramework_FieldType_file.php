...
class FeedZapper_AdminPageFramework_FieldType_text extends FeedZapper_AdminPageFramework_FieldType {
    public $aFieldTypeSlugs = array('text', 'password', 'date', 'datetime', 'datetime-local', 'email', 'month', 'search', 'tel', 'url', 'week',);
    protected $aDefaultKeys = array();
    protected function getStyles() {
        return ".feed-zapper-field.feed-zapper-field-text > .feed-zapper-input-label-container {vertical-align: middle; }.feed-zapper-field.feed-zapper-field-text > .feed-zapper-input-label-container.feed-zapper-field-text-multiple-labels {display: block;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'feed-zapper-field-text-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='feed-zapper-input-label-container {$_sClassSelector}'>", "<label for='" . $_aInputAttributes['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'feed-zapper-input-label-string') . ">" . $_sLabel . "</span>" : '', "<input " . $this->getAttributes($_aInputAttributes) . " />", $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
}
class FeedZapper_AdminPageFramework_FieldType_file extends FeedZapper_AdminPageFramework_FieldType_text {
