...
class FeedZapper_AdminPageFramework_FieldType_checkbox extends FeedZapper_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    jQuery( '{$_sClassSelectorSelectAll}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_all_button_container\" onclick=\"jQuery( this ).selectAllFeedZapper_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_all_button' ) + '</a></div>' );
    });            
    jQuery( '{$_sClassSelectorSelectNone}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_none_button_container\" onclick=\"jQuery( this ).deselectAllFeedZapper_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_none_button' ) + '</a></div>' );
    });
});
JAVASCRIPTS;
    ...
    private function _getSelectButtonClassSelectors(array $aFieldTypeSlugs, $sDataAttribute = 'select_all_button') {
        ...
        foreach ($aFieldTypeSlugs as $_sSlug) {
            ...
            $_aClassSelectors[] = '.feed-zapper-checkbox-container-' . $_sSlug . "[data-{$sDataAttribute}]";
        }
        return implode(',', $_aClassSelectors);
    }
    protected function getStyles() {
        return ".select_all_button_container, .select_none_button_container{display: inline-block;margin-bottom: 0.4em;}.feed-zapper-checkbox-label {margin-top: 0.1em;}.feed-zapper-field input[type='checkbox' ] {margin-right: 0.5em;} .feed-zapper-field-checkbox .feed-zapper-input-label-container {padding-right: 1em;}.feed-zapper-field-checkbox .feed-zapper-input-label-string{display: inline; }";
    }
...
class FeedZapper_AdminPageFramework_FieldType_posttype extends FeedZapper_AdminPageFramework_FieldType_checkbox {
    ...
    protected function getStyles() {
        ...
        return $_sParentStyles . ".feed-zapper-field input[type='checkbox'] {margin-right: 0.5em;} .feed-zapper-field-posttype .feed-zapper-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
