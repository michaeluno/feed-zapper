...
class FeedZapper_AdminPageFramework_FieldType_select extends FeedZapper_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".feed-zapper-field-select .feed-zapper-input-label-container {vertical-align: top; }.feed-zapper-field-select .feed-zapper-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        $_oSelectInput = new FeedZapper_AdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'feed-zapper-input-label-container feed-zapper-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class FeedZapper_AdminPageFramework_FieldType_size extends FeedZapper_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".feed-zapper-field-size input {text-align: right;}.feed-zapper-field-size select.size-field-select {vertical-align: 0px; }.feed-zapper-field-size label {width: auto; } .form-table td fieldset .feed-zapper-field-size label {display: inline-block;}";
    }
    protected function getField($aField) {
    ...
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'feed-zapper-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new FeedZapper_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
