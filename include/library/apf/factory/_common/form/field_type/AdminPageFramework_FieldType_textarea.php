...
class FeedZapper_AdminPageFramework_FieldType_textarea extends FeedZapper_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    var updateFoundEditors = function( oElements ) {
        ...
        oElements.each( function( iIndex ) {
            ...
            var oSettings = jQuery().getFeedZapper_AdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
    ...
    jQuery().registerFeedZapper_AdminPageFrameworkCallbacks( {	
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            var _oSettings = jQuery().getFeedZapper_AdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.feed-zapper-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            setTimeout(function(){
                var _oFields = oSortedFields.children( '.feed-zapper-field' );
        ...
        stopped_sorting_sections: function( oSections ) {
            ...
            setTimeout(function(){
                var _oFields = jQuery( oSections ).find( '.feed-zapper-field' );
                updateFoundEditors( _oFields );           
            }, 100 );
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.feed-zapper-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storeFeedZapper_AdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return ".feed-zapper-field-textarea .feed-zapper-input-label-string {vertical-align: top;margin-top: 2px;} .feed-zapper-field-textarea .wp-core-ui.wp-editor-wrap {margin-bottom: 0.5em;}.feed-zapper-field-textarea.feed-zapper-field .feed-zapper-input-label-container {vertical-align: top; float: left;clear: both;} .postbox .feed-zapper-field-textarea .feed-zapper-input-label-container {width: 100%;}.feed-zapper-field-textarea textarea {max-width: 100%;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'feed-zapper-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='feed-zapper-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'feed-zapper-input-label-string') . ">" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='feed-zapper-textarea-enabler'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
